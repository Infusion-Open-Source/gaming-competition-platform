namespace Infusion.Gaming.LightCycles.Serialization
{
    ...
    public class MapGenerator : IMapProvider
    {
        ...
        /// <param name="obstacleRatio">Ratio of obstacles on a map to generate</param>
        public MapGenerator(string mapName, int width, int height, int numberOfPlayers, int numberOfTeams, float obstacleRatio)
        {
        ...
        /// <summary>
        /// Gets or sets ratio of obstacles on the map to generate
        /// </summary>
        ...
        /// <summary>
        /// Creates players team assignments
        /// </summary>
        ...
        protected MapLocation[,] GenerateLocationsBox(int width, int height)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    else if (random.NextDouble() < this.ObstacleRatio)
                    {
                        locations[x, y] = new Obstacle(p);
                    }
                    else
